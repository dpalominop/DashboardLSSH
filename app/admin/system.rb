ActiveAdmin.register System do
  ...
  batch_action :destroy, confirm: I18n.t("active_admin.batch_confirm_system")  do |ids|
    ids = ids.map { |i| i.to_i }
    batch_action_collection.find(ids.flatten).each do |resource|
      resource.destroy
    end
    if ids.size == 1 then
      redirect_to collection_path, notice: I18n.t("active_admin.batch_destroy_system")
    else
      redirect_to collection_path, notice: I18n.t("active_admin.batch_destroy_systems")
    end
  end
